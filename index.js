...
function startRecursive() {
  ...
  // 		const promArr = users.slice(index, index + 10).map((itm) => processUser(itm.id));
  // 		const result = await Promise.all(promArr);
  		
  // 		total += result.reduce((acc, cv) => acc + cv);
  ...
  return new Promise((resolve, reject) => {
  	getUsers()
  	function getUsers(index = 0, total = 0) {
  		const promArr = users.slice(index, index + 10).map((itm) => processUser(itm.id));
  		Promise.all(promArr).then(result => {
  			total += result.reduce((acc, cv) => acc + cv)
  			
  			if (index + 10 < users.length) {
  				getUsers(index + 10, total);
	  		}
	  		else resolve(total);
  		});		 
  	} 
